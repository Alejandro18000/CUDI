...
# Premium Liquid Glass AI Widget Template
...
def purge_and_reinject():
    ...
    for filename in html_files:
        ...
        # 1. Broad Purge: Remove anything that looks like the widget or its fragments
        # Remove the main widget block
        content = re.sub(r'<!-- Global AI Chat Widget.*?</div>\s*</div>\s*</div>', '', content, flags=re.DOTALL)
        content = re.sub(r'<div class="cudi-chat-widget".*?</div>\s*</div>\s*</div>', '', content, flags=re.DOTALL)
        
        # Remove the floating "horrible bar" footer fragment specifically
        content = re.sub(r'<div class="chat-panel-footer".*?</div>', '', content, flags=re.DOTALL)

        # 2. Clean Reinjection: Place it exactly once after <body>
        content = re.sub(r'(<body.*?>)', r'\1\n' + AI_WIDGET_TEMPLATE, content, flags=re.IGNORECASE)

        with open(filename, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"Purged & Injected: {filename}")
        count += 1
